...
class Mapper {
 public:
  ...
  void increment_internal_clock(u8 amount, const u16& RTC_DAY) {
    ...
    internal_clock += amount;

    // fmt::println("[MBC3] internal clock: {}", internal_clock);
    if (internal_clock >= 4194304) {
      internal_clock -= 4194304;
      tick_rtc();
    }
  }
  ...
  struct RTC_REGS {
    ...
    void write_to_active_reg(RTC_REGISTER r, u8 v, u64& internal_clock) {
      switch (r) {
        case RTC_REGISTER::RTC_SECOND_TIME: {
          ...
          // fmt::println("wrote {:#08x} to RTC_SECOND_TIME {:#08x}", v, (u8)RTC_SECOND_TIME);
          internal_clock  = 0;
          break;
        }
    ...
    u16 read_from_active_reg(RTC_REGISTER r) {
      switch (r) {
        case RTC_REGISTER::RTC_SECOND_TIME: {
