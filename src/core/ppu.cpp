  ...
  if (this->ppu_mode != new_mode) {
    ...
    switch (ppu_mode) {
      case RENDERING_MODE::HBLANK: {
        ...
        if (lcdc.sprite_enable == 1) {
          for (auto sprite : sprite_buf) {
            ...
            for (u8 x = 0; x < 8; x++) {
              ...
              u32 buf_pos = (current_x) + ((bus->io.data[LY] + TOP_PADDING) * 256);
        ...
        //     // we're done
        //     fmt::println("TRANSFER FINISHNED");
      ...
      case RENDERING_MODE::VBLANK: {
        ...
        if (frame_skip) {
          frame_skip = false;
          fmt::println("frame skipped after re-enabling");
          break;
        }
        frame_queued = true;
        break;
